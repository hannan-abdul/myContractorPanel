...
import { createShop, getShopProfile, updateShop, } from '../../services/Shop';
...
const Shops = ({ id, closeModal, closeEdit }) => {
  ...
  const AddShop = () => {
    ...
    createShop(JSON.stringify(data))
    .then((response) => {
      if (response) {
  ...
  const EditShop = () => {
    ...
    debugger
    updateShop(JSON.stringify(data), id)
    .then((response) => {
      if (response) {
        window.location.reload()
        //closeModal(false)
